...
buildProtocurl() {
  ...
  {
    echo ""
    grep "^FROM " release/builder.Dockerfile
    # add inotify to binaries to test tmp-file permissions. also add pkill for cleanup
      echo "RUN apt-get update && apt-get install -y inotify-tools procps"
      echo "# ============="
      echo "FROM $PROTOCURL_IMAGE_ORIGINAL as final"
      echo "COPY --from=builder /bin/* /bin/"
      echo "COPY --from=builder /usr/bin/* /usr/bin/"
      echo "
      ...
      "
      grep "^ENTRYPOINT " release/final.Dockerfile
  } >"$TMP_DOCKERFILE"
  remove-leading-spaces-inplace "$TMP_DOCKERFILE"

  docker build --target final -t "$PROTOCURL_IMAGE" -q -f - . <"$TMP_DOCKERFILE"
  echo "Done."
}
